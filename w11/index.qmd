...
## Judging Split Options {.smaller .crunch-title .crunch-ul .crunch-code .crunch-math .fwtables}

Let's think through two choices for the first split:

::: {layout="[10,-1,10]" layout-valign="center" layout-align="center"}
...
0 &\text{if }\text{latitude} \leq 47.5, \\
0 &\text{otherwise}
\end{cases}
$$
...
## Scikit-Learn: Growing the Tree {.smaller .crunch-title .crunch-quarto-layout-panel .crunch-math .smaller-math .fwtables .crunch-code}

::: {layout="[[1,2],[1,2]]" layout-valign="top" layout-align="center"}
...
$$
\begin{align*}
\mathscr{L}(R_1) &= -\left[ \frac{13}{25}\log_2\left(\frac{13}{25}\right) + \frac{12}{25}\log_2\left(\frac{12}{25}\right) \right] \approx 0.999 \\
\mathscr{L}(R_2) &= -\left[ \frac{61}{75}\log_2\left(\frac{61}{75}\right) + \frac{14}{75}\log_2\left(\frac{14}{75}\right) \right] \approx 0.694 \\
...
## Chopping the Feature Space Always Reduces Entropy (!) {.smaller}

* Visual proof:

```{r}
#| label: lower-entropy-proof
library(tidyverse)
