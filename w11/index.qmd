...
latitude <- seq(-90, 90, by=5)
ski_df <- expand_grid(day, latitude)
#ski_df |> head()
# Data-generating process
ski_df <- ski_df |> mutate(
  near_equator = abs(latitude) <= 30,
  northern = latitude > 30,
  southern = latitude < -30,
...
ggplot(ski_sample, aes(x=day, y=latitude, shape=good_skiing, color=good_skiing)) +
  geom_point(
...
ski_sample <- ski_sample |> mutate(
  lat_gt_n30 = latitude >= -30
)
ski_sample |> group_by(lat_gt_n30) |> count(good_skiing)
```
