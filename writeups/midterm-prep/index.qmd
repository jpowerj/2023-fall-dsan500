...
We can use the Pandas function `pd.melt()`, however, to **convert this dataset from long to wide format**
...
3. What Pandas function could we use to transform the dataset from the previous problem into the dataset from the first problem?
    a. `pd.sort_values()`
    b. `pd.melt()`
    c. `pd.to_numeric()`
    d. `pd.unique()`
